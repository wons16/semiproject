...
$.giftstore_fixObj = function( _this, $contents_wrap, $category_content, $giftstore_logo){ // 카테고리 메뉴 & 받은선물/장바구니
    var isScroll;
...
$.giftstore_targetBanner = function( _targetClassName, _colorObj ){ // 타켓배너
    var _target = _targetClassName;
    var _length = $('.' + _target + ' ul li').length;
...
$.giftstore_hotdeal = function( _targetClassName, _cntObj ){ // 핫딜영역
    ...
    switch( _targetClassName ){
        case 'category_product_detail_wrap':    // 상세
            viewCnt = 1;
    ...
    function initSetDisplayCnt(_target){
        ...
        for(var i = 0; i < cntObjLen; i++){
            ...
            var $hotdealRestItems = $target.children('.hotdeal_rest_items');
             $hotdealRestItems.text(_target[i].restItems);    // E 남은 수량
...
$.giftstore_giftRecommend = function( _targetClassName ){ // 선물추천 10
    var _target = _targetClassName;
    var _length = $('.' + _target + ' ul li').length;
...
$.giftstore_togetherView = function( _targetClassName ){ // 이런상품 어때요
    var _target = _targetClassName;
    var _length = $('.' + _target + ' ul li').length;
...
$.comFormRadio = function(_checkClassName){
    ...
    $target.find('input[type="radio"]').on({
        change:function(e){
            var _target = e.target;
            if($(_target).hasClass('kakaopay')){ //  카카오 페이
